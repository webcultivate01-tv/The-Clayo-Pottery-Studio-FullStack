...
$typeMeta = [
    'service'  => ['label' => 'Service',  'cls' => 'bg-brand-50  text-brand   border-brand-100'],
    'workshop' => ['label' => 'Workshop', 'cls' => 'bg-accent-50 text-accent  border-accent/20'],
];
?>
...
<div class="mb-5 flex items-center justify-between flex-wrap gap-3">
    <div>
        <h2 class="font-display text-2xl font-bold text-ink">Services &amp; Workshops</h2>
    ...
    <button onclick="openAddModal()"
        ...
        <i data-lucide="plus" class="w-4 h-4"></i> Add Service / Workshop
    </button>
</div>

<!-- Filter panel -->
<div class="bg-panel rounded-xl border border-line p-4 mb-5 shadow-card">
    <form method="GET" action="<?= e(url('/services')) ?>">
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            <div>
                <label class="block text-[11px] font-semibold uppercase tracking-[0.12em] text-muted mb-1.5">Type</label>
                <select name="type"
...
<?php if (empty($items)): ?>
    <div class="bg-panel rounded-xl border border-line shadow-card py-16 text-center">
        <i data-lucide="sparkles" class="w-10 h-10 text-subtle mx-auto mb-3"></i>
        <p class="text-[13px] font-semibold text-ink">No items found</p>
        <p class="text-[12px] text-muted mt-1">
            <?= array_filter($filters, fn($v) => $v !== '') ? 'Try adjusting your filters.' : 'Add your first service or workshop to get started.' ?>
        </p>
    </div>
<?php else: ?>
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        <?php foreach ($items as $it):
            $tm  = $typeMeta[$it['type']] ?? $typeMeta['service'];
        ...
        ?>
            <div class="bg-panel rounded-xl border border-line shadow-card overflow-hidden flex flex-col group">
                <div class="aspect-[16/10] bg-line-soft relative overflow-hidden">
                    ...
                    <?php endif; ?>
                    <span class="absolute top-2.5 left-2.5 inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-semibold border <?= $tm['cls'] ?>">
                        <?= $tm['label'] ?>
                    </span>
                ...
                <div class="p-4 flex-1 flex flex-col">
                    ...
                    <?php endif; ?>
                    <div class="mt-3 flex items-center gap-3 text-[11px] text-ink-soft">
                        ...
                        <?php endif; ?>
                        <?php if (!empty($it['duration'])): ?>
                            <span class="inline-flex items-center gap-1"><i data-lucide="clock" class="w-3 h-3 text-subtle"></i><?= e($it['duration']) ?></span>
                        <?php endif; ?>
                    </div>
        ...
        <div class="flex items-center justify-between mt-5 px-2">
            ...
            <div class="flex items-center gap-1">
                <?php for ($i = 1; $i <= $pages; $i++):
                    $url_i  = url('/services') . '?' . http_build_query(array_filter($filters, fn($v) => $v !== '') + ['page' => $i]);
                    $active = $i === $page;
                ?>
                    <a href="<?= e($url_i) ?>"
    ...
    <div class="absolute inset-y-0 right-0 w-full max-w-md flex flex-col bg-panel shadow-pop">
        <div class="flex items-center justify-between px-6 py-4 border-b border-line shrink-0">
            <div class="flex items-center gap-2.5">
                ...
                <div>
                    <h3 class="font-display text-[15px] font-bold text-ink">Add Service / Workshop</h3>
        ...
        <form action="<?= e(url('/services/store')) ?>" method="POST" enctype="multipart/form-data"
            ...
            <div>
                ...
                <input type="text" name="title" required placeholder="e.g. Wheel Throwing Workshop" class="form-input">
            </div>
            <div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="price" placeholder="e.g. ₹1,200" class="form-input">
                </div>
                <div>
                    <label class="form-label">Duration</label>
                    <input type="text" name="duration" placeholder="e.g. 2 hours" class="form-input">
                </div>
            </div>
            <div>
    ...
    <div class="absolute inset-y-0 right-0 w-full max-w-md flex flex-col bg-panel shadow-pop">
        <div class="flex items-center justify-between px-6 py-4 border-b border-line shrink-0">
            <div class="flex items-center gap-2.5">
                ...
                <div>
                    <h3 class="font-display text-[15px] font-bold text-ink">Edit</h3>
                    <p class="text-[11px] text-muted" id="editSubtitle">Update item</p>
                </div>
            </div>
...
<div id="deleteModal" class="fixed inset-0 z-50 hidden flex items-center justify-center p-4">
    ...
    <div class="relative bg-panel rounded-2xl shadow-pop w-full max-w-sm p-6 text-center">
        ...
        <h3 class="font-display text-[16px] font-bold text-ink mb-1">Delete this item?</h3>
...
function openEditModal(it) {
    ...
    f.action = BASE + '/services/' + it.id + '/update';
    document.getElementById('edit_type').value        = it.type        || 'service';
    document.getElementById('edit_title').value       = it.title       || '';
    document.getElementById('edit_price').value       = it.price       || '';
    document.getElementById('edit_duration').value    = it.duration    || '';
    ...
    document.getElementById('editSubtitle').textContent = it.title || 'Update item';
...
function openDeleteModal(id, name) {
    document.getElementById('deleteForm').action = BASE + '/services/' + id + '/delete';
